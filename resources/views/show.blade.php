...
@extends('layouts.main') {{-- Đảm bảo tên layout của bạn là 'layouts.main' --}}

@section('title', $product->name ?? 'Chi tiết sản phẩm') {{-- Đặt tiêu đề trang là tên sản phẩm --}}
        ...
        <div class="row">
            <div class="col-md-12">
                <ul class="breadcrumb-tree">
                    ...
                    @if($product->category)
                        {{-- Tương tự như product.blade.php, nếu muốn link danh mục hoạt động, cần thêm route --}}
                        <li><a href="{{ route('store.index', ['category' => $product->category->slug ?? '']) }}">{{ $product->category->name ?? 'N/A' }}</a></li>
                    @else
                        <li><a href="#">Danh mục không xác định</a></li>
                    @endif
                    <li class="active">{{ $product->name ?? 'Sản phẩm' }}</li>
                </ul>
            </div>
        </div>
    </div>
